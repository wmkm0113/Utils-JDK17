...
public abstract class BeanObject implements Serializable {
    ...
    public final String toString(final StringUtils.StringType stringType, final boolean formatOutput) {
	    return switch (stringType) {
		    case XML -> this.toXML(formatOutput);
		    case JSON -> formatOutput ? this.toFormattedJson() : this.toJson();
		    case YAML -> formatOutput ? this.toFormattedYaml() : this.toYaml();
		    case SERIALIZABLE -> StringUtils.objectToString(this, StringUtils.StringType.SERIALIZABLE, formatOutput);
